...
Ext.form.ux.touch.MultiSelect = Ext.extend(Ext.form.Select, {
    ...
    destroyItemPanel: function() {
		...
		this.fireEvent('finalchange',this);
    ...
    showComponent: function() {
        ...
        if (itemType === "picker") {
			...
			var index = [];
			var mvalue = (this.value).split(', ');
			for(var i=0;i<mvalue.length;i++){
				index[i] = this.store.findExact(this.valueField, mvalue[i]);
			}
            itemPanel.showBy(this.el, "fade", false);
			for(var j=0;j<index.length;j++){
				if(index[j] != -1) itemPanel.down('#list').getSelectionModel().select(index[j],true);
			}
        }
        this.isActive = true;
    },
